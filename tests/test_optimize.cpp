...
constexpr auto
rosenbrock_function_term(const auto_diff::variable<double> &alpha,
...
constexpr auto
rosenbrock_function_helper(const expr_t &e,
                           const auto_diff::variable<double> &alpha,
...
template <size_t dim> constexpr auto rosenbrock_function() {
  static_assert(
      dim >= 2,
  ...
  const auto_diff::variable<double> alpha(0), x0(1), x1(2);
  const auto e = rosenbrock_function_term(alpha, x0, x1);
  if constexpr (dim == 2) {
    return e;
  } else {
    return rosenbrock_function_helper<dim - 2>(e, alpha, 3);
  }
}

TEST(nonlinear_optimizer, rosenbrock_function2) {
  constexpr auto f = rosenbrock_function<2>();
}

TEST(nonlinear_optimizer, rosenbrock_function16) {
  constexpr auto f = rosenbrock_function<16>();
}
